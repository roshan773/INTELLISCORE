  ...
  return (
    <div className="relative w-full">
      ...
      <div className="fixed top-6 left-0 right-0 z-50 flex justify-center px-4 sm:px-6 pointer-events-none">
        <header
          className={`pointer-events-auto transition-all duration-500 rounded-full flex items-center justify-between px-4 sm:px-6 py-2.5 max-w-5xl w-full ${
            isScrolled
        ...
        >
          ...
          >
            <div className="w-8 h-8 rounded-full flex items-center justify-center text-brand-olive-dark font-display font-black text-sm group-hover:rotate-45 transition-transform duration-300 overflow-hidden">
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.name}
                  href={link.href}
                  className={`font-display text-xs uppercase tracking-widest py-1 relative group transition-colors duration-200 ${
                    isActive
                      ? "text-brand-cream font-bold"
                      : "text-brand-cream/70 hover:text-brand-cream font-medium"
                  }`}
                >
                  <span>{link.name}</span>
                  <span
                    className={`absolute bottom-0 left-0 h-[2px] bg-brand-cream transition-all duration-300 ${
          ...
          <div className="flex items-center gap-3">
            <MagneticButton
              ...
              className={`text-xs px-5 sm:px-6 py-2 sm:py-2.5 rounded-full ${
                pathname === "/contact" ? "ring-2 ring-brand-cream shadow-lg" : ""
              }`}
            >
      ...
      {/* Mobile Drawer */}
      <div
        className={`fixed inset-0 z-40 bg-brand-olive-dark/98 backdrop-blur-2xl lg:hidden transition-all duration-500 flex flex-col justify-between p-8 pt-32 ${
          mobileMenuOpen
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  className={`font-display text-2xl uppercase tracking-wide flex items-center justify-between py-2 border-b transition-colors ${
                    isActive
                      ? "text-brand-cream font-bold border-brand-cream"
                      : "text-brand-cream/80 hover:text-brand-cream border-brand-cream/10"
                  }`}
                >
                  <span>{link.name}</span>
                  <ArrowUpRight className={`w-5 h-5 ${isActive ? "text-brand-cream" : "text-brand-olive-light"}`} />
                </Link>
              );
            })}
          </nav>
